...
class Product extends Component {
  ...
  handleRemove = (id) => {
    MySwal.fire({
      title: 'Bạn có chắt không?',
      text: "Nếu bạn xoá thì không thể khôi phục",
      icon: 'warning',
              ...
              <div className="col-lg-12">
                <div className="card">
                  <div className="card-header d-flex align-items-center">
                    <h3 className="h4">Dữ liệu bảng sản phẩm</h3>
                          ...
                          {products && products.length ? products.map((item, index) => {
                            return (
                              <tr key={index}>
                                ...
                                <td style={{ textAlign: "center" }}>
                                  <div>
                                    <span title='Sửa' className="fix-action"><Link to={`/products/edit/${item.id}`}> <i className="fa fa-edit"></i></Link></span>
                                    <span title='Xóa' onClick={() => this.handleRemove(item.id)} className="fix-action"><Link to="#"> <i className="fa fa-trash" style={{ color: '#ff00008f' }}></i></Link></span>
                                  </div>
                                </td>
                              </tr>
                            )
